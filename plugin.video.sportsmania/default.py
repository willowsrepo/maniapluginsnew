...
cookie_jar = os.path.join(cookie_path, THESITE+"14_amember_new_matrix.lwp")
...
def checksub():
   ...
   html = net.http_GET('http://sportsmania.eu/user_test.php').content
...
def Login():
    ...
    try:
       ...
       net.set_cookies(cookie_jar)
       dp.update(100,"Login into SPORTSMANIA.... ", "Please Wait")
       html = net.http_POST(loginurl, data, headers).content
       dp.close()
    ...
    if 'true' in html:
       ...
       net.set_cookies(cookie_jar)
       sub = net.http_GET('http://sportsmania.eu/user_test.php').content
       subs=json.loads(sub)
       stat = False
       for field in subs:
            status=field['status'].title()
       ...
       if stat:		
            ...
            dialog = xbmcgui.Dialog()
            dialog.ok(THESITE.upper(), '','Your subscription has expired please go to http://sportsmania.eu/payments/login to resubscribe to SPORTS MANIA','')
            try:os.remove(cookie_jar)
            except:pass
            SYSEXIT()
            return False
...
def CATEGORIES():
    ...
    if os.path.exists(cookie_jar) == True:
        try:
            ...
            addDir('.[COLOR royalblue]*** [/COLOR] [COLOR orange]' + status.strip() + '[/COLOR]','url',1,MEDIA_URL+'twitter.png', '', '', '')
        except:pass
        ...
        addDir('[COLOR beige].View Calendar[/COLOR]','http://sportsmania.eu/sites/ukcalendar.html',2005,MEDIA_URL+'calendar.png','','','')
        addDir('[COLOR beige].View USA Calendar[/COLOR]','http://sportsmania.eu/sites/usacalendar.html',2005,MEDIA_URL+'test.png','','','')
        ...
        addDir('[COLOR gold].Willows Sports Mix Section 2 (Football Rugby & More)[/COLOR]','http://'+THESITE+'/apisprotected/plp2.php',2003,MEDIA_URL+'extra.png','','','')
...
def addDir(name,url,mode,iconimage,play,date,description,page=''):
        ...
        if mode == 2 or mode==7  or mode==2004 or mode==3002 or mode==16001 or mode==16002 or mode==16003 or mode==16004 or mode==16005 or mode==16006 or mode==203 or mode==202 or mode==16008:
            if not mode == 2000:
                if not '.f4m' in url:
                    liz.setProperty("IsPlayable","true")
